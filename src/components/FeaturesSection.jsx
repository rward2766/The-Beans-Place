...
function ImageRow( { images,offset = 0} ) {
    ...
    return (
        <div className="carusel-row" style= { {transform: `translate3d (${offset}px, 0, 0)`}}>
        ...
        const handleScroll = () => {
            ...
            const range = Math.min(window.innerHeight * 0.5, 600);
                setOffsets([
                    ...
                    p * range - range, // row 2: slifes right (starts offset left)
                    -p * range * 0.7 // row 3: rslides left slower
                ])
        };
        handleScroll();
